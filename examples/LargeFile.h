...
#    endif // __cpp_lib_concepts
  ...
  _Vector_const_iterator( _Tptr                   _Parg,
                          const _Container_base * _Pvector ) noexcept
      : _Ptr( _Parg )
  {
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _STL_VERIFY( _Ptr < _Mycont->_Mylast,
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _STL_VERIFY( _Mycont->_Myfirst < _Ptr,
...
#    if _ITERATOR_DEBUG_LEVEL == 0
    (void)_Off;
#    else  // ^^^ _ITERATOR_DEBUG_LEVEL == 0 ^^^ // vvv _ITERATOR_DEBUG_LEVEL !=
           // 0 vvv
    const auto _Mycont = static_cast<const _Myvec *>( this->_Getcont() );
    _STL_VERIFY( _Off == 0 || _Ptr,
                 "cannot seek value-initialized vector iterator" );
    if ( _Off < 0 ) {
      _STL_VERIFY( _Off >= _Mycont->_Myfirst - _Ptr,
                   "cannot seek vector iterator before begin" );
    }

    if ( _Off > 0 ) {
      _STL_VERIFY( _Off <= _Mycont->_Mylast - _Ptr,
...
#    endif // _ITERATOR_DEBUG_LEVEL == 0
  ...
  _NODISCARD _Vector_const_iterator
  ...
  _NODISCARD difference_type
  operator-( const _Vector_const_iterator & _Right ) const
  {
    _Compat( _Right );
...
template<class _Myvec>
_NODISCARD _Vector_const_iterator<_Myvec>
           operator+( typename _Vector_const_iterator<_Myvec>::difference_type _Off,
...
struct pointer_traits<_Vector_const_iterator<_Myvec>>
{
  ...
  _NODISCARD static constexpr element_type *
  to_address( const pointer _Iter ) noexcept
  {
#      if _ITERATOR_DEBUG_LEVEL != 0
    // A value-initialized iterator is in the domain of to_address. An
    // invalidated end iterator for a vector with capacity() of 0 is not. This
    // function cannot distinguish those two cases, so it incorrectly does not
    // diagnose the latter. In practice, this isn't a significant problem since
    // to_address returns nullptr for such an iterator.
    const auto _Mycont = static_cast<const _Myvec *>( _Iter._Getcont() );
    if ( _Mycont ) {
      _STL_VERIFY( _Mycont->_Myfirst <= _Iter._Ptr &&
                       _Iter._Ptr <= _Mycont->_Mylast,
                   "can't convert out-of-range vector iterator to pointer" );
    }
    else {
      _STL_VERIFY( !_Iter._Ptr,
...
template<class _Myvec>
_NODISCARD _Vector_iterator<_Myvec>
           operator+( typename _Vector_iterator<_Myvec>::difference_type _Off,
...
struct _Value_init_tag
{ // tag to request value-initialization
...
auto
_Unfancy_maybe_null( _Ptrty _Ptr )
...
public:
  static_assert( !_ENFORCE_MATCHING_ALLOCATORS ||
                     is_same_v<_Ty, typename _Alloc::value_type>,
...
private:
  template<class _Ty2>
  void _Construct_n_copies_of_ty( _CRT_GUARDOVERFLOW const size_type _Count,
                                  const _Ty2 &                       _Val )
  {
...
public:
  explicit vector( _CRT_GUARDOVERFLOW const size_type _Count,
  ...
  vector( _CRT_GUARDOVERFLOW const size_type _Count,
          const _Ty &                        _Val,
          const _Alloc &                     _Al = _Alloc() )
      : _Mypair( _One_then_variadic_args_t {}, _Al )
  {
...
private:
  ...
  void _Range_construct_or_tidy( _Iter _First, _Iter _Last, input_iterator_tag )
  {
    ...
    for ( ; _First != _Last; ++_First ) {
      emplace_back( *_First ); // performance note: emplace_back()'s strong
                               // guarantee is unnecessary here
    }
  ...
  template<class _Iter>
  void
  _Range_construct_or_tidy( _Iter _First, _Iter _Last, forward_iterator_tag )
  {
    const auto _Count = _Convert_size<size_type>(
...
public:
      ...
      : _Mypair( _One_then_variadic_args_t {}, _Al )
  {
    ...
    _Range_construct_or_tidy(
        _Ilist.begin(), _Ilist.end(), random_access_iterator_tag {} );
    _Proxy._Release();
  }
    ...
    if _CONSTEXPR_IF ( !_Alty_traits::is_always_equal::value ) {
      if ( _Getal() != _Right._Getal() ) {
        ...
        if ( _Rightfirst != _Rightlast ) {
          ...
          auto &              _My_data = _Mypair._Myval2;
          _My_data._Mylast =
              _Umove( _Rightfirst, _Rightlast, _My_data._Myfirst );
          _Guard._Target = nullptr;
        }
        return;
      }
...
public:
  ...
  vector( vector && _Right, const _Alloc & _Al ) noexcept(
...
private:
  ...
  void _Move_assign( vector & _Right,
                     _Propagate_allocators ) noexcept /* terminates */
  {
    _Tidy();
#    if _ITERATOR_DEBUG_LEVEL != 0
    if ( _Getal() != _Right._Getal() ) {
      ...
      _Mypair._Myval2._Reload_proxy(
          _GET_PROXY_ALLOCATOR( _Alty, _Getal() ),
          _GET_PROXY_ALLOCATOR( _Alty, _Right._Getal() ) );
    }
#    endif
  ...
  void _Move_assign( vector & _Right, _No_propagate_allocators )
  {
    if ( _Getal() == _Right._Getal() ) {
      ...
      const auto _Oldcapacity =
          static_cast<size_type>( _My_data._Myend - _Myfirst );
#    if _HAS_IF_CONSTEXPR
      if constexpr ( conjunction_v<
                         bool_constant<
                             _Ptr_copy_cat<_Ty *, _Ty *>::_Trivially_copyable>,
                         _Uses_default_construct<_Alty, _Ty *, _Ty>,
                         _Uses_default_destroy<_Alty, _Ty *>> ) {
        if ( _Newsize > _Oldcapacity ) {
          _Clear_and_reserve_geometric( _Newsize );
        }

        _Mylast = _Refancy<pointer>( _Copy_memmove(
            _Unfancy( _First ), _Unfancy( _Last ), _Unfancy( _Myfirst ) ) );
      }
      else
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _Delete_plain_internal(
        _Alproxy, _STD exchange( _Mypair._Myval2._Myproxy, nullptr ) );
#    endif // _ITERATOR_DEBUG_LEVEL != 0
  }
...
private:
  ...
  {
    // insert by perfectly forwarding into element at end, provide strong
    // guarantee
    auto &    _My_data = _Mypair._Myval2;
    pointer & _Mylast  = _My_data._Mylast;
    _STL_INTERNAL_CHECK(
        _Mylast != _My_data._Myend ); // check that we have unused capacity
    _Alty_traits::construct(
        _Getal(), _Unfancy( _Mylast ), _STD forward<_Valty>( _Val )... );
...
public:
  ...
  decltype( auto ) emplace_back( _Valty &&... _Val )
  {
    ...
    if ( _Mylast != _My_data._Myend ) {
      return _Emplace_back_with_unused_capacity(
...
#    endif // _HAS_CXX17
  ...
  pointer _Emplace_reallocate( const pointer _Whereptr, _Valty &&... _Val )
  {
    ...
    _STL_INTERNAL_CHECK(
        _Mylast == _My_data._Myend ); // check that we have no unused capacity
    ...
    _TRY_BEGIN
    _Alty_traits::construct(
        _Al, _Unfancy( _Newvec + _Whereoff ), _STD forward<_Valty>( _Val )... );
...
#    if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY( _Where._Getcont() == _STD addressof( _My_data ) &&
                     _Whereptr >= _My_data._Myfirst && _Oldlast >= _Whereptr,
...
#    endif // _ITERATOR_DEBUG_LEVEL == 2
    ...
    if ( _Oldlast != _My_data._Myend ) {
      if ( _Whereptr == _Oldlast ) { // at back, provide strong guarantee
        ...
        auto &                  _Al = _Getal();
        _Alloc_temporary<_Alty> _Obj(
            _Al, _STD forward<_Valty>( _Val )... ); // handle aliasing
        // after constructing _Obj, provide basic guarantee
        _Orphan_range( _Whereptr, _Oldlast );
        _Alty_traits::construct(
            _Al, _Unfancy( _Oldlast ), _STD move( _Oldlast[-1] ) );
        ++_My_data._Mylast;
    ...
    return _Make_iterator(
        _Emplace_reallocate( _Whereptr, _STD forward<_Valty>( _Val )... ) );
  }
...
#    if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY( _Where._Getcont() == _STD addressof( _My_data ) &&
                     _Whereptr >= _Oldfirst && _Oldlast >= _Whereptr,
                 "vector insert iterator outside range" );
#    endif // _ITERATOR_DEBUG_LEVEL == 2

    const auto _Whereoff = static_cast<size_type>( _Whereptr - _Oldfirst );
    const auto _Unused_capacity =
        static_cast<size_type>( _My_data._Myend - _Oldlast );
    ...
    if ( _Count == 0 ) { // nothing to do, avoid invalidating iterators
      ...
      const _Alloc_temporary<_Alty> _Tmp_storage( _Getal(),
                                                  _Val ); // handle aliasing
      const auto &                  _Tmp = _Tmp_storage._Storage._Value;
      const auto                    _Affected_elements =
          static_cast<size_type>( _Oldlast - _Whereptr );
      _Orphan_range( _Whereptr, _Oldlast );
...
private:
  template<class _Iter>
  void _Insert_range( const_iterator _Where,
                      _Iter          _First,
                      _Iter          _Last,
                      input_iterator_tag )
  {
    ...
    // Otherwise, provide basic guarantee (despite N4659 26.3.11.5
    // [vector.modifiers]/1). Performance note: except for one-at-back,
    // emplace_back()'s strong guarantee is unnecessary here.
                      ...
                      forward_iterator_tag )
  {
    ...
    if ( _Count == 0 ) { // nothing to do, avoid invalidating iterators
      ...
      if ( _Count == 1 &&
           _Whereptr == _Oldlast ) { // one at back, provide strong guarantee
        _Umove_if_noexcept( _Oldfirst, _Oldlast, _Newvec );
      }
      else { // provide basic guarantee
        _Umove( _Oldfirst, _Whereptr, _Newvec );
    ...
    else { // Attempt to provide the strong guarantee for EmplaceConstructible
           // failure. If we encounter copy/move construction/assignment
           // failure, provide the basic guarantee. (For one-at-back, this
      ...
      if ( _Count <
           _Affected_elements ) { // some affected elements must be assigned
        _Mylast = _Umove( _Oldlast - _Count, _Oldlast, _Oldlast );
        _Move_backward_unchecked( _Whereptr, _Oldlast - _Count, _Oldlast );
...
#    if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY( _Where._Getcont() == _STD addressof( _My_data ) &&
                     _Whereptr >= _Oldfirst && _My_data._Mylast >= _Whereptr,
                 "vector insert iterator outside range" );
#    endif // _ITERATOR_DEBUG_LEVEL == 2

    _Adl_verify_range( _First, _Last );
    const auto _Whereoff = static_cast<size_type>( _Whereptr - _Oldfirst );
    _Insert_range( _Where,
...
private:
  ...
  { // assign input range [_First, _Last)
    ...
    // Code size optimization: we've exhausted only the source, only the dest,
    // or both. If we've exhausted only the source: we Trim, then Append does
    // nothing. If we've exhausted only the dest: Trim does nothing, then we
    // Append. If we've exhausted both: Trim does nothing, then Append does
    // nothing.
...
#    if _HAS_IF_CONSTEXPR
    if constexpr (
        conjunction_v<
            bool_constant<_Ptr_copy_cat<_Iter, _Ty *>::_Trivially_copyable>,
      ...
      _Mylast = _Refancy<pointer>(
          _Copy_memmove( _First, _Last, _Unfancy( _Myfirst ) ) );
    }
    else
#    endif // _HAS_IF_CONSTEXPR
    {
      ...
      if ( _Newsize > _Oldsize ) {
        ...
        const _Iter _Mid =
            _STD    next( _First, static_cast<difference_type>( _Oldsize ) );
...
public:
  ...
  {
    _Adl_verify_range( _First, _Last );
    _Assign_range( _Get_unwrapped( _First ),
                   _Get_unwrapped( _Last ),
                   _Iter_cat_t<_Iter> {} );
  }
...
private:
  ...
  {
    // set capacity to _Newcapacity (without geometric growth), provide strong
    // guarantee
    auto &    _My_data = _Mypair._Myval2;
    pointer & _Myfirst = _My_data._Myfirst;
    pointer & _Mylast  = _My_data._Mylast;
...
#    if _ITERATOR_DEBUG_LEVEL != 0 && defined( _ENABLE_STL_INTERNAL_CHECK )
    _STL_INTERNAL_CHECK( _Newsize != 0 );
    {
      ...
      _STL_INTERNAL_CHECK(
          !_My_data._Myproxy
               ->_Myfirstiter ); // asserts that all iterators are orphaned
    }                            // unlock
#    endif // _ITERATOR_DEBUG_LEVEL != 0 && defined(_ENABLE_STL_INTERNAL_CHECK)
    ...
    if ( _Myfirst ) { // destroy and deallocate old array
      ...
      _Getal().deallocate( _Myfirst,
                           static_cast<size_type>( _Myend - _Myfirst ) );
...
public:
  ...
  {
    // increase capacity to _Newcapacity (without geometric growth), provide
    // strong guarantee
    if ( _Newcapacity >
         capacity() ) { // something to do (reserve() never shrinks)
...
#    if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY( _Where._Getcont() == _STD addressof( _My_data ) &&
                     _Whereptr >= _My_data._Myfirst && _Mylast > _Whereptr,
...
#    if _ITERATOR_DEBUG_LEVEL == 2
    _STL_VERIFY( _First._Getcont() == _STD    addressof( _My_data ) &&
                     _Last._Getcont() == _STD addressof( _My_data ) &&
                     _Firstptr >= _My_data._Myfirst && _Lastptr >= _Firstptr &&
                     _Mylast >= _Lastptr,
                 "vector erase iterator outside range" );
#    endif // _ITERATOR_DEBUG_LEVEL == 2
...
public:
  ...
  _NODISCARD _Ty &
             operator[]( const size_type _Pos ) noexcept /* strengthened */
  {
    auto & _My_data = _Mypair._Myval2;
#    if _CONTAINER_DEBUG_LEVEL > 0
    _STL_VERIFY(
        _Pos < static_cast<size_type>( _My_data._Mylast - _My_data._Myfirst ),
...
#    endif // _CONTAINER_DEBUG_LEVEL > 0
  ...
  _NODISCARD const _Ty &
                   operator[]( const size_type _Pos ) const noexcept /* strengthened */
  {
    auto & _My_data = _Mypair._Myval2;
#    if _CONTAINER_DEBUG_LEVEL > 0
    _STL_VERIFY(
...
#    endif // _CONTAINER_DEBUG_LEVEL > 0
  ...
  {
    auto & _My_data = _Mypair._Myval2;
    if ( static_cast<size_type>( _My_data._Mylast - _My_data._Myfirst ) <=
         _Pos ) {
      _Xrange();
    }
  ...
  _NODISCARD _Ty & front() noexcept /* strengthened */
  {
    auto & _My_data = _Mypair._Myval2;
#    if _CONTAINER_DEBUG_LEVEL > 0
    _STL_VERIFY( _My_data._Myfirst != _My_data._Mylast,
...
private:
  ...
  void
  _Umove_if_noexcept1( pointer _First, pointer _Last, pointer _Dest, true_type )
  {
    // move [_First, _Last) to raw _Dest, using allocator
    _Uninitialized_move( _First, _Last, _Dest, _Getal() );
  }

  void _Umove_if_noexcept1( pointer _First,
                            pointer _Last,
                            pointer _Dest,
                            false_type )
  {
  ...
  void _Umove_if_noexcept( pointer _First, pointer _Last, pointer _Dest )
  {
    ...
    _Umove_if_noexcept1(
        ...
        _Last,
        _Dest,
        bool_constant<disjunction_v<is_nothrow_move_constructible<_Ty>,
                                    negation<is_copy_constructible<_Ty>>>> {} );
  }
  ...
  void _Buy_raw( const size_type _Newcapacity )
  {
    ...
    _STL_INTERNAL_CHECK( !_Myfirst && !_Mylast &&
                         !_Myend ); // check that *this is tidy
...
#    else  // ^^^ _ITERATOR_DEBUG_LEVEL == 2 ^^^ // vvv _ITERATOR_DEBUG_LEVEL !=
           // 2 vvv
    (void)_First;
    (void)_Last;
#    endif // _ITERATOR_DEBUG_LEVEL == 2
...
template<class _Iter,
         ...
         enable_if_t<conjunction_v<_Is_iterator<_Iter>, _Is_allocator<_Alloc>>,
                     int> = 0>
...
void
swap( vector<_Ty, _Alloc> & _Left,
      vector<_Ty, _Alloc> & _Right ) noexcept /* strengthened */
{
  _Left.swap( _Right );
}
...
template<class _Ty, class _Alloc>
_NODISCARD bool
operator==( const vector<_Ty, _Alloc> & _Left,
            const vector<_Ty, _Alloc> & _Right )
{
  return _Left.size() == _Right.size() &&
         _STD equal( _Left._Unchecked_begin(),
...
operator<( const vector<_Ty, _Alloc> & _Left,
           const vector<_Ty, _Alloc> & _Right )
{
  return _STD lexicographical_compare( _Left._Unchecked_begin(),
                                       _Left._Unchecked_end(),
                                       _Right._Unchecked_begin(),
...
public:
  ...
  _Vb_iter_base( const _Vbase *          _Ptr,
                 _Size_type              _Off,
                 const _Container_base * _Mypvbool ) noexcept
      : _Myptr( _Ptr )
      , _Myoff( _Off )
  {
...
#    if _ITERATOR_DEBUG_LEVEL != 0
  _Difference_type _Total_off( const _Mycont * _Cont ) const
  {
    return static_cast<_Difference_type>(
        _VBITS * ( _Myptr - _Cont->_Myvec.data() ) + _Myoff );
  }
#    endif // _ITERATOR_DEBUG_LEVEL != 0
...
public:
  ...
  _Vb_reference( const _Mybase & _Right ) noexcept
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _STL_VERIFY( _Cont,
                 "cannot dereference value-initialized vector<bool> iterator" );
    _STL_VERIFY( this->_Total_off( _Cont ) <=
                     static_cast<_Difference_type>( _Cont->_Mysize ),
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    if ( _Off != 0 ) {
      const auto _Cont = static_cast<const _Mycont *>( this->_Getcont() );
      _STL_VERIFY( _Cont,
                   "cannot seek value-initialized vector<bool> iterator" );
      const auto _Start_offset = this->_Total_off( _Cont );
      if ( _Off < 0 ) {
        _STL_VERIFY( -_Start_offset <= _Off,
                     "cannot seek vector<bool> iterator before begin" );
      }
      else if ( 0 < _Off ) {
        _STL_VERIFY( _Off <= static_cast<_Difference_type>( _Cont->_Mysize -
                                                            _Start_offset ),
...
#    endif // _ITERATOR_DEBUG_LEVEL != 0
    ...
    if ( _Off < 0 && this->_Myoff < 0 - static_cast<_Size_type>(
                                            _Off ) ) { // add negative increment
      this->_Myoff += _Off;
      this->_Myptr -=
          1 + ( static_cast<_Size_type>( -1 ) - this->_Myoff ) / _VBITS;
      this->_Myoff %= _VBITS;
    }
  ...
  _NODISCARD difference_type
  operator-( const _Vb_const_iterator & _Right ) const
  {
    _Compat( _Right );
    return static_cast<difference_type>( _VBITS *
...
#    if _ITERATOR_DEBUG_LEVEL != 0
  ...
  friend void _Verify_range( const _Vb_const_iterator & _First,
                             const _Vb_const_iterator & _Last )
  {
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _STL_VERIFY( this->_Total_off( _Cont ) > 0,
                 "cannot decrement vector<bool> begin iterator" );
#    endif // _ITERATOR_DEBUG_LEVEL != 0
...
template<class _Alvbase_wrapped>
_NODISCARD _Vb_const_iterator<_Alvbase_wrapped>
           operator+( typename _Vb_const_iterator<_Alvbase_wrapped>::difference_type _Off,
...
template<class _Alvbase_wrapped>
_NODISCARD _Vb_iterator<_Alvbase_wrapped>
           operator+( typename _Vb_iterator<_Alvbase_wrapped>::difference_type _Off,
...
public:
  ...
  _Vb_val( const _Alloc & _Al ) noexcept(
      is_nothrow_constructible_v<_Vectype, _Alvbase> )
  ...
  _Vb_val( size_type _Count, const bool & _Val, const _Alloc & _Al )
      : _Myvec( _Nw( _Count ),
                static_cast<_Vbase>( _Val ? -1 : 0 ),
                static_cast<_Alvbase>( _Al ) )
      , _Mysize( 0 )
  {
    this->_Alloc_proxy( _GET_PROXY_ALLOCATOR( _Alvbase, _Getal() ) );
  }
  ...
  _Vb_val( _Vb_val && _Right ) noexcept(
      is_nothrow_move_constructible_v<_Vectype> )
      : _Myvec( _STD move( _Right._Myvec ) )
      , _Mysize( _STD exchange( _Right._Mysize, size_type { 0 } ) )
  {
    this->_Alloc_proxy( _GET_PROXY_ALLOCATOR( _Alvbase, _Getal() ) );
  }

  _Vb_val( _Vb_val && _Right, const _Alloc & _Al ) noexcept(
...
#    if _ITERATOR_DEBUG_LEVEL != 0
    ...
    _Delete_plain_internal( _Alproxy,
                            _STD exchange( this->_Myproxy, nullptr ) );
#    endif // _ITERATOR_DEBUG_LEVEL != 0
  }
...
public:
  static_assert( !_ENFORCE_MATCHING_ALLOCATORS ||
                     is_same_v<bool, typename _Alloc::value_type>,
                 _MISMATCHED_ALLOCATOR_MESSAGE( "vector<bool, Allocator>",
                                                "bool" ) );
  ...
  vector() noexcept(
      is_nothrow_default_constructible_v<_Mybase> ) // strengthened
      : _Mybase()
  {
  }
  ...
  explicit vector( _CRT_GUARDOVERFLOW size_type _Count,
                   const _Alloc &               _Al = _Alloc() )
  ...
  vector( _CRT_GUARDOVERFLOW size_type _Count,
          const bool &                 _Val,
          const _Alloc &               _Al = _Alloc() )
  ...
  vector( vector && _Right ) noexcept(
      is_nothrow_move_constructible_v<_Mybase> ) // strengthened
...
#    if _ITERATOR_DEBUG_LEVEL != 0
  ...
  void _Move_assign( vector & _Right, _Propagate_allocators ) noexcept
  {
    ...
    if ( this->_Getal() != _Right._Getal() ) { // reload proxy
      ...
      _Container_proxy_ptr<_Alvbase> _Proxy( _Right_proxy_al,
...
#    endif // _ITERATOR_DEBUG_LEVEL == 0
  ...
  _NODISCARD size_type max_size() const noexcept
  {
    constexpr auto _Diff_max =
        static_cast<size_type>( ( numeric_limits<difference_type>::max )() );
    const size_type _Ints_max = this->_Myvec.max_size();
    if ( _Ints_max >
         _Diff_max / _VBITS ) { // max_size bound by difference_type limits
      return _Diff_max;
    }
  ...
  _NODISCARD const_reference
  operator[]( size_type _Off ) const noexcept /* strengthened */
  {
#    if _CONTAINER_DEBUG_LEVEL > 0
...
#    endif // _CONTAINER_DEBUG_LEVEL > 0
  ...
  {
    _Adl_verify_range( _First, _Last );
    auto _Count = _Convert_size<size_type>(
        static_cast<size_t>( _STD distance( _First, _Last ) ) );
    size_type _Off = _Insert_x( _Where, _Count );
    _Copy_unchecked( _Get_unwrapped( _First ),
                     _Get_unwrapped( _Last ),
...
#    endif // _ITERATOR_DEBUG_LEVEL == 2
  ...
  iterator erase( const_iterator _First_arg,
                  const_iterator _Last_arg ) noexcept /* strengthened */
  {
...
#    if _ITERATOR_DEBUG_LEVEL == 2
      _STL_VERIFY( _Last >= _First && end() >= _Last,
...
#    endif // _ITERATOR_DEBUG_LEVEL == 2
  ...
  iterator
  _Insert_n( const_iterator _Where, size_type _Count, const bool & _Val )
  {
    size_type  _Off    = _Insert_x( _Where, _Count );
    const auto _Result = begin() + static_cast<difference_type>( _Off );
    _STD       fill(
        _Result, _Result + static_cast<difference_type>( _Count ), _Val );
    return _Result;
  }
...
#    if _ITERATOR_DEBUG_LEVEL == 2
      _Orphan_range( static_cast<size_type>( _Realloc ? 0 : _Off ),
                     this->_Mysize );
#    endif // _ITERATOR_DEBUG_LEVEL == 2
    }
...
#    if _ITERATOR_DEBUG_LEVEL == 2
  void _Orphan_range( size_type _Offlo, size_type _Offhi ) const
  {
    ...
    while ( *_Pnext ) { // test offset from beginning of vector
      ...
      const auto   _Off =
          static_cast<size_type>( _VBITS * ( _Pnextiter._Myptr - _Base ) ) +
          _Pnextiter._Myoff;
...
#    endif // _ITERATOR_DEBUG_LEVEL == 2
  ...
  void _Trim( size_type _Size )
  {
    ...
    if ( _Words < this->_Myvec.size() ) {
      this->_Myvec.erase( this->_Myvec.begin() +
                              static_cast<difference_type>( _Words ),
                          this->_Myvec.end() );
    }
...
template<class _Alloc>
_NODISCARD bool
operator==( const vector<bool, _Alloc> & _Left,
...
{
  _CXX17_DEPRECATE_ADAPTOR_TYPEDEFS typedef vector<bool, _Alloc>
                                                   _ARGUMENT_TYPE_NAME;
  _CXX17_DEPRECATE_ADAPTOR_TYPEDEFS typedef size_t _RESULT_TYPE_NAME;

  _NODISCARD size_t
  operator()( const vector<bool, _Alloc> & _Keyval ) const noexcept
  {
    return _Hash_array_representation( _Keyval._Myvec.data(),
                                       _Keyval._Myvec.size() );
  }
};
...
typename vector<_Ty, _Alloc>::size_type
erase( vector<_Ty, _Alloc> & _Cont, const _Uty & _Val )
{
...
typename vector<_Ty, _Alloc>::size_type
erase_if( vector<_Ty, _Alloc> & _Cont, _Pr _Pred )
{
